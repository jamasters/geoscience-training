...
leafletDirective.directive("leaflet", ["$http", "$log", "$parse",
function($http, $log, $parse) {
   ...
   return {
      ...
      link : function($scope, element, attrs /*, ctrl */) {
         ...
         $scope.leaflet.doubleClickZoom = false;
         ...
         var tileLayerObj = L.tileLayer($scope.leaflet.tileLayer, defaults.tileLayerOptions);
         tileLayerObj.addTo(map);
         $scope.leaflet.tileLayerObj = !!attrs.testing ? tileLayerObj : 'Add testing="testing" to <leaflet> tag to inspect this object';
         ...
         function buildMarker(name, data) {
            var marker = new L.marker($scope.markers[name], {
             ...
             if(data.description && data.description.length > 40) {
                 data.description =  data.description.substring(0,39)+"...";
             }

            var output = '' +
                '<a class="leaflet-popup-close-button pull-right" href="#close"><i class="icon-remove-sign icon-2x"></i></a>' +
                '<div class="container-fluid">' +
                 ...
                 '<div class="row-fluid">' +
                  '<div class="">' +
                   '<img class="img-rounded" style="height:135px; width: 135px" src="' + data.imageUrl + '" alt="">' +
                  '</div>' +
                '</div>' +
                '<div class="row-fluid">' +
            ...
            marker.bindPopup(output, {
               ...
               maxHeight : '200px',
               maxWidth : '300px',
               autoPanPadding : new L.Point(40, 35)
            });
         ...
         function createPath(name, scopePath, map) {
            ...
            if (scopePath.weight !== undefined) {
               polyline.setStyle({
                  weight : scopePath.weight
               });
            }

            if (scopePath.color !== undefined) {
               polyline.setStyle({
                  color : scopePath.color
               });
            }

            if (scopePath.opacity !== undefined) {
               polyline.setStyle({
                  opacity : scopePath.opacity
               });
            }
